...
START_TEST(only_uppercase_test) {
...
Suite *suite_s21_to_upper(void) {
  ...
  TCase *tcase_core = tcase_create("core of s21_to_upper");

  tcase_add_test(tcase_core, regular_test);
  tcase_add_test(tcase_core, mixed_case_test);
  tcase_add_test(tcase_core, only_uppercase_test);
